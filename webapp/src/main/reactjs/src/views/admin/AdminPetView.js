...
function Content({ pet, id }) {
    ...
    const { breeds, species, provinces, departments } = useContext(
        ConstantsContext
    );
    ...
    return (
        <>
            ...
            <h2>
                {isMale(gender)
                    ? t("someInformationAboutHim")
                    : t("someInformationAboutHer")}
                :
            </h2>
            ...
            <List bordered={true}>
                ...
                <ListItemRow
                    name={t("details.dateOfBirth")}
                    value={moment(birthDate).format("DD/MM/YYYY")}
                />
                <ListItemRow
                    name={t("details.species")}
                    value={speciesId && species[speciesId].name}
                />
                <ListItemRow
                    name={t("details.breed")}
                    value={breedId && breeds[breedId].name}
                />
                <ListItemRow
                    name={t("details.sex")}
                    value={t("details." + _.toLower(gender))}
                />
                <ListItemRow
                    name={t("details.province")}
                    value={provinceId && provinces[provinceId].name}
                />
                <ListItemRow
                    name={t("details.department")}
                    value={departmentId && departments[departmentId].name}
                />
                <ListItemRow
                    name={t("details.uploadDate")}
                    value={moment(uploadDate).format("DD/MM/YYYY")}
                />
                <ListItemRow
                    name={t("details.owner")}
                    value={<Link to={ADMIN_USER + userId}>{username}</Link>}
                />
                <ListItemRow
                    name={t("status.vaccinated")}
                    value={vaccinated ? <CheckOutlined /> : <CloseOutlined />}
                />
            </List>
            ...
            <div>
                <Link to={ADMIN_HOME}>{t("backToHome")}</Link>
            </div>
        </>
    );
}
    ...
    if (pet.status === 0 || pet.status === 3) {
        ...
        modalButton = (
            <Button
                ...
                danger
                key={"remove"}
                onClick={() => showModal(onConfirm, modalMessage)}
            >
                {t("buttons.remove")}
            </Button>
        );
        ...
        <ContentWithHeader
            content={
                <div>
                    ...
                    >
                        <div>{modalState.modalMessage}</div>
                    </Modal>
                </div>
            }
            actionComponents={[
            ...
            title={
                petName ? (
                    petName + " (" + statusLocale[pet.status] + ")"
                ) : (
                    <Spin />
                )
            }
